...
class EyeHandEnv(gym.Env):
  ...
  def __init__(self, # task
    ...
    step_cost,train, fitts_W=None, fitts_D=None,
    ):
  ...
  def reset(self):
    self.correct=0
    self.latency=np.array([-1.0,-1.0])
    self.episode_steps=0

    # random target position
    if self.train:
      self.fitts_W=np.random.uniform(low=0.02, high=0.1)
      self.fitts_D=np.random.uniform(low=0.15, high=0.5)
  ...
  def _reward_function(self,action_index):
    ...
    if action_index==ACTION_CLICK: # terminal action
      ...
      if action_index==ACTION_NOOP:
        ...
        reward=self.step_cost if self.stage[EYE]==FIXATE else self.step_cost*3
      elif action_index==ACTION_NEW_HAND_COMMAND:
        reward=self.step_cost if self.stage[HAND]==STILL else self.step_cost*3
  ...
  def _no_new_command(self,mode):
    ...
    if previous_stage==PREP and self.stage[mode]!=PREP and self.first[mode]:
      # complete prep of the first submovement
      self.first[mode]=False
      self.latency[mode]=self.episode_steps
