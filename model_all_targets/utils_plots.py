...
def MT_learned_behaviour(model,env,n_eps):
    '''
    run the trained model, return the behaviours on each step
    '''
    eps=0
    MTs=np.array((n_eps))
    latencies==np.array((n_eps,2))
    correct=0
    while eps<n_eps:
        obs=env.reset()
        done=False
        while not done:
            ...
            obs, reward, done, info = env.step(action)
            if done:
                MTs[eps]=env.episode_steps
                latencies[eps,:]=env.latency
                correct+=env.correct
                eps+=1
    return np.mean(MTs), np.std(MTs), correct/n_eps, np.mean(latencies)
